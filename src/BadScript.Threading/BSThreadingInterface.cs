...
namespace BadScript.Threading
{
    public class BSThreadingInterface : ABSScriptInterface
    {
        ...
        private static ABSTable CreateThreadObject(Thread task)
        {
            ...
            t.InsertElement("Interrupt", new BSFunction("function Interrupt()", args => InterruptThread(task), 0));
            t.InsertElement("Join", new BSFunction("function Join()/Join(msTimeout)", args => JoinThread(task, args), 0,1));
            t.InsertElement("Start", new BSFunction("function Start()", args => 
                                                                            StartThread(task), 0));
            t.Lock();
        ...
        private static ABSTable CreateTaskObject( Task < ABSObject > task )
        {
            ...
            t.SetRawElement("IsCompleted", new BSReflectionReference(() => 
                                                                         task.IsCompleted ? BSObject.True : BSObject.False, null));
            ...
            t.InsertElement("Wait", new BSFunction("function Wait()/Wait(msTimeout)", arg => WaitTask(task, arg), 0, 1));
            t.Lock();
        ...
        public override void AddApi( ABSTable root )
        {
            ...
            root.InsertElement( "CurrentThread", new BSReflectionReference( () => CreateThreadObject(Thread.CurrentThread), null ) );
            root.InsertElement("CreateThread", new BSFunction("function CreateThread(fnc, args..)", CreateThread, 1, int.MaxValue));
            root.InsertElement("CreateTask", new BSFunction("function CreateTask(fnc, args..)", CreateTask, 1, int.MaxValue));
