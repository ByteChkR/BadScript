...
namespace BadScript.Common.Namespaces
{
    ...
    public class BSNamespace
    {
        ...
        public BSNamespace GetNamespace(string name) =>
            m_Children.First(x => x.Name == name);

        public virtual bool ContainsType( string name, bool includeChildren) => HasType( name, includeChildren) || Parent.ContainsType( name, false );
        ...
        public virtual BSClassExpression ResolveType( string name, bool includeChildren )
        {
            ...
            throw new BSRuntimeException($"Can not find Class '{name}' in namespace '{FullName}' are you missing a using statement?");
        }

        public void AddType( BSClassExpression expr ) => m_Types.Add( expr );

        public void AddUsing( BSNamespace ns ) => m_IncludedNamespaces.Add( ns );
        ...
        protected BSClassExpression GetType( string name, bool includeChildren)
        {
            ...
            {
                if (ns.HasType(name, false)) return ns.GetType(name,false);
            }
            if(includeChildren&& m_Children.Any(x => x.HasType(name, true)))
            foreach (BSNamespace ns in m_Children)
            {
                if (ns.HasType(name, true)) return ns.GetType(name, true);
            }
